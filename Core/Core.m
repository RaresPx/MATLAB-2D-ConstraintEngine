classdef Core < handle
    ...
    methods
        function obj = Core()
          % CORE - Create Core controller for simulation and rendering
          %
          % Initializes figure, axes, timing and scene objects.
        ...
        function run(obj)
          % RUN - Main loop: step simulation, render, and maintain timing
            while obj.Running && isvalid(obj.FigureHandle)
                loopTic = tic;
                % ---- Simulation step ----
                if ~obj.Paused
                    % Advance physics by fixed timestep dt
